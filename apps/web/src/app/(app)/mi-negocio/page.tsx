...
import { z } from 'zod'
import { BUSINESS_CATEGORIES } from '@klikeo/shared'
import Navbar from '@/components/Navbar'
import { useRequireAuth } from '@/lib/hooks/useRequireAuth'
import { dashboardClient, NegocioDashboard } from '@/lib/dashboard-client'
...
const CATEGORY_LABELS: Record<string, string> = {
  alimentos: 'Alimentos', ropa: 'Ropa', salud: 'Salud', servicios: 'Servicios',
  tecnologia: 'Tecnología', educacion: 'Educación', belleza: 'Belleza', hogar: 'Hogar',
  deportes: 'Deportes', entretenimiento: 'Entretenimiento', transporte: 'Transporte',
  turismo: 'Turismo', mascotas: 'Mascotas', construccion: 'Construcción', otros: 'Otros',
}
...
export default function MiNegocioPage() {
  ...
  useEffect(() => {
    ...
    dashboardClient.negocios.getByOwner(token)
      .then((n) => {
        setNegocio(n)
        reset({ name: n.name, description: n.description, category: n.category, city: n.city, whatsappNumber: n.whatsappNumber, address: n.address, phone: n.phone })
      })
      .catch(() => setFetchError(''))
  }, [token, reset])
  ...
  const onSubmit = async (data: NegocioForm) => {
    ...
    try {
      ...
      if (negocio) {
        ...
        updated = await dashboardClient.negocios.create({ ...data, whatsappNumber: data.whatsappNumber }, token)
      }
      setNegocio(updated)
  ...
  if (loading) return <div className="p-16 text-center text-muted">Cargando...</div>
      ...
      <div className="max-w-2xl mx-auto py-10 px-6">
        ...
        <div className="bg-surface border border-border rounded-xl p-8">
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
            {[
              { field: 'name' as const, label: 'Nombre del negocio *', type: 'text' },
              { field: 'city' as const, label: 'Ciudad *', type: 'text' },
              { field: 'whatsappNumber' as const, label: 'Número de WhatsApp *', type: 'tel' },
              { field: 'phone' as const, label: 'Teléfono (opcional)', type: 'tel' },
              { field: 'address' as const, label: 'Dirección (opcional)', type: 'text' },
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5 text-text">Categoría *</label>
              <select
                {...register('category')}
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5 text-text">Descripción (opcional)</label>
              <textarea
            ...
            >
              {isSubmitting ? 'Guardando...' : negocio ? 'Guardar cambios' : 'Crear negocio'}
            </button>
          </form>
        </div>
